...
def rank_by_blended_momentum(df_top_n):
    """
    第三層攻擊過濾 (終極防禦與黃金坑版)：
    計算多重時間框架綜合動能分數，並加入「月線乖離率」偵測短期錯殺黃金坑。
    """
    print(f"\n⚡ 啟動進階大腦決策：正在計算前 {len(df_top_n)} 檔 ETF 的「三維綜合動能分數」與「乖離率」...")

    momentum_data = []

    for index, row in df_top_n.iterrows():
        code = row['Code']
        ticker_symbol = f"{code}.TW" if not code.endswith('B') else f"{code}.TWO"

        # 1. 預設值全數初始化為 0.0
        roc_20d, roc_60d, roc_120d, blended_score = 0.0, 0.0, 0.0, 0.0
        bias_20 = 0.0  # 新增：月線乖離率
        is_golden_pit = "" # 新增：黃金坑標記
        ...
        try:
            ...
            if len(hist) > 0:
                ...
                if len(hist) >= 20:
                    ...
                    # 💡 黃金坑核心邏輯：計算 20MA 與乖離率
                    ma20 = hist['Close'].tail(20).mean()
                    if ma20 > 0:
                        bias_20 = ((latest_price - ma20) / ma20) * 100
                        # 設定觸發門檻：對於原型 ETF，跌破月線 4% 算嚴重超跌
                        if bias_20 <= -4.0:
                            is_golden_pit = "🚨 超跌黃金坑"
        ...
        bias_20 = 0.0 if pd.isna(bias_20) else bias_20
...
if __name__ == "__main__":
    ...
    if now_in_taipei.weekday() == 0:
        ...
        safe_etf_list = fetch_and_filter_etf(min_scale_ntd=40000000000, top_n=20)

        if safe_etf_list is not None and not safe_etf_list.empty:

            # 1. 取得今日最新分數
            df_final = rank_by_blended_momentum(safe_etf_list)
            ...
            # --- 測試執行區塊 ---
            print(f"\n🎯 最終決策清單 (120日長線保護 + 黃金坑偵測)：")
            # 💡 把乖離率和訊號標記加入顯示清單
            display_cols = ['Code', 'ETF名稱', '近120日漲幅(%)', '近20日漲幅(%)', '月線乖離率(%)', '綜合動能分數', '訊號標記']
            existing_cols = [c for c in display_cols if c in df_final.columns]

            print(f"\n📊 目前排序依據：【綜合動能分數】")
            # 💡 建立一個專供顯示用的 DataFrame，並依照 120 日漲幅降冪 (由大到小) 排序
            df_display = df_final.sort_values(by='綜合動能分數', ascending=False)
